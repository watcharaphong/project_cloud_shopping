...
require('../config/config.php');
	...
	while ($data = mysqli_fetch_array($rs, MYSQLI_BOTH)) {
		$i++;
		$sum = $data['oprice'] * $data['ounit'] ;
		@$total += $sum;
?>
  <tr>
    <td><?=$i;?></td>
    <td><img src="../<?php echo $data['image'];?>"width="50" height="50"> <br>

	รหัสสินค้า: <?=@$data['code'];?> <br>
	ชื่อสินค้า: <?=$data['name'];?></td>
    <td><?=$data['ounit'];?></td>
    <td><?=number_format($data['oprice'],0);?></td>
