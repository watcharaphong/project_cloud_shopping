<?php
//การใช้งาน sesstion เพื่อเช็คสินค้าในตะกร้า
session_start();

// นำไฟล์เข้าสู้ฐานข้อมูลในตะกร้า
require_once("config/dbcontroller.php");
require('config/config.php');
$db_handdle = new DBController();

//ตรวจสอบการทำงานของลิ้งค์
$select_mixed = "select max(ordersid) from orders";
$send_mixed = mysqli_query($link, $select_mixed);
$rec_mixed = mysqli_fetch_array($send_mixed) or die ("หาไอดีสูงสุดไม่ได้");
$id_max = $rec_mixed[0];
...
if(!empty($_GET["action"])){
	...
	//ค้นหาข้อมูลที่ถูกเลืกลงในตะกร้า
	$productByCode = $db_handle->runQuery("SELECT * FROM tbproduct WHERE code = '". $_GET["code"]."'");
	...
	//ถ้าสินค้าซ้ำให้เลืกเพิ่มได้อีก
                ...
                <div class="aa-footer-widget">
                  <div class="aa-footer-widget">
                    ...
                    <address>
                      ...
                      <p><span class="fa fa-phone"></span>+66 65265 1390</p>
                      <p><span class="fa fa-envelope"></span>63010974017@msu.ac.th</p>
                    </address>
